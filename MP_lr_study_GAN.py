...
df_I = transf.fillna_frac_min_feature(df_initial, fraction=0.2)
...
for lbl in ['FTMP', 'DMP', 'UCB']:
    ...
    for i in range(4):
        ...
        df_storage_test[lbl][i+1] = (test_datamatrix_G - df_G.mean())/np.sqrt(df_G.std())

# Linear interpolation of data
data, lbls = laf.artificial_dataset_generator(df_storage_train['FTMP'][1], labels=lbl_storage_train['FTMP'][1],
                                                max_new_samples_per_label=384, binary=False, rnd=list(np.linspace(0.15,0.9,6)), 
...
generator.save_weights('gan_models/MPint_gen_opt_lr_'+ lr_str)
...
with open('gan_models/MPint_results_opt_lr_' + lr_str + '.pickle', 'wb') as handle:
