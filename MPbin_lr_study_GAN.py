...
test_lrs = ('1e-5', '5e-5', '1e-4', '5e-4', '1e-3', '5e-3')
...
print(f"\nINFO: Job with array task id {idx} is using lr={lr_str}")
...
def training_montage(train_data_o, train_lbls, test_data, test_lbls,
    ...
    test_data = (test_data).values#/(2*test_data.values.std())).values
...
generator = generator_model(data.shape[1], data.shape[1], 256, 3)
critic = critic_model(data.shape[1], 512, 3)
...
generator.save_weights('gan_models/MP_gen_opt_lr_'+ lr_str)
...
with open('gan_models/MP_results_opt_lr_' + lr_str + '.pickle', 'wb') as handle:
