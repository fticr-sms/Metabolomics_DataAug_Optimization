...
data, lbls = laf.artificial_dataset_generator(real_samples, labels=target,
                                                    max_new_samples_per_label=90, binary='random sampling', 
...
def training_montage(train_data_o, train_lbls, test_data, test_lbls,
    ...
    for step in range(n_steps):
        ...
        if (step + 1) % update2 == 0:
            ...
            f_samp = generate_predictions(generator, 112, all_data.shape[1], input_realdata_dist, ordered_labels)
...
generator = generator_model(data.shape[1], data.shape[1], 256, 7)
critic = critic_model(data.shape[1], 512, 7)
...
generator.save_weights('gan_models/YDbin_gen_opt_lr_'+ lr_str)
...
with open('gan_models/YDbin_results_opt_lr_' + lr_str + '.pickle', 'wb') as handle:
