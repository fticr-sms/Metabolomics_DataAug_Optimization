...
# All targets
target = ['HuCC','HuCC','HuCC','HuCC','HuCC',#'HuCC','HuCC','HuCC','HuCC','HuCC',
          'HuDC','HuDC','HuDC','HuDC','HuDC',
         'HgCC','HgCC','HgCC','HgCC','HgCC',
          'HgDC','HgDC','HgDC','HgDC','HgDC',
         'LtCC','LtCC','LtCC','LtCC','LtCC','LtCC','LtCC','LtCC','LtCC','LtCC',
          'LtDC','LtDC','LtDC','LtDC','LtDC',
         'MpCC','MpCC','MpCC','MpCC','MpCC',
          'MpDC','MpDC','MpDC','MpDC','MpDC','MpDC','MpDC','MpDC','MpDC','MpDC',
         'ScCC','ScCC','ScCC','ScCC','ScCC','ScCC','ScCC','ScCC','ScCC','ScCC',
          'ScDC','ScDC','ScDC','ScDC','ScDC','ScDC','ScDC','ScDC','ScDC','ScDC',
         'SbCC','SbCC','SbCC','SbCC','SbCC',
          'SbDC','SbDC','SbDC','SbDC','SbDC',
          'ZbCC','ZbCC','ZbCC','ZbCC','ZbCC','ZbCC','ZbCC','ZbCC','ZbCC','ZbCC',
...
for cl in classes:
    for col in file.columns:
        if col.startswith(cl[:2]):
            if col[3:5] == cl[2:]:
                if cl == 'HuCC' and col.endswith('_2'):
                    #print(col)
                    continue
...
# Actually make the target correct
target = ['CC','CC','CC','CC','CC',####'CC','CC','CC','CC','CC',
          'DC','DC','DC','DC','DC',
         'CC','CC','CC','CC','CC',
         'DC','DC','DC','DC','DC',
...
filt_sample_data = basic_feat_filtering(file.T, target, filt_method='total_samples', filt_kw=2)
real_samples = df_to_bool(filt_sample_data)

labels = target

data, lbls = laf.artificial_dataset_generator(real_samples, labels=labels,
...
generator = generator_model(data.shape[1], data.shape[1], 256, 2)
critic = critic_model(data.shape[1], 512, 2)

batch_size = 32

n_epochs = 600

training_montage(data, lbls, data, lbls, n_epochs, generator, critic, generator_optimizer, critic_optimizer,
                     input_realdata_dist, batch_size, grad_pen_weight=5, k_cov_den=20, k_crossLID=15)

# Save the generator models' weights.
generator.save_weights('gan_models/YDbinCCvsDC_gen_opt_lr_'+ lr_str)
...
with open('gan_models/YDbinCCvsDC_results_opt_lr_' + lr_str + '.pickle', 'wb') as handle:
