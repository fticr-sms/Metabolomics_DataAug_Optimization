...
data, lbls = laf.artificial_dataset_generator(imputed, labels=target,
                                        max_new_samples_per_label=256, binary=False, rnd=list(np.linspace(0.2,0.8,3)), 
...
for crit_n_str in test_nodes:
    ...
    generator.save_weights('gan_models/YDintCCvsDC_gen_opt_gen_'+ gen_nodes_str + 'crit_' + crit_n_str)
    ...
    with open('gan_models/YDintCCvsDC_results_opt_gen_'+ gen_nodes_str + 'crit_' + crit_n_str + '.pickle', 'wb') as handle:
