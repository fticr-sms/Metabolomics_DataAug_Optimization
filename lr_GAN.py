...
# Sleeping based on index of array so every job can individually fetch the data in hdf5 store without conflict
import time
time.sleep(idx*120)
...
training_montage(data_NGP, lbls, data_NGP, lbls, n_epochs, generator,
...
generator.save_weights('gan_models/GD_gen_opt_lr_'+ lr_str)
...
with open('gan_models/GD_results_opt_lr_' + lr_str + '.pickle', 'wb') as handle:
