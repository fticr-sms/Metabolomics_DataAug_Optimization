...
# Sleeping absed on index of array so every job can individually fetch the data in hdf5 store without conflict
import time
time.sleep(idx*120)
...
for lbl in ['FTMP', 'DMP', 'UCB']:
    ...
    for i in range(4):
        ...
        for cl in permutations.keys():
            if cl == lbl:
                train_idxs[cl] = list(np.array(permutations[cl])[i*14: (i+1)*14])
                test_idxs[cl] = list(np.array(permutations[cl])[: i*14]) + list(np.array(permutations[cl])[(i+1)*14:])
            else:
                train_idxs[cl] = list(np.array(permutations[cl])[: i*14]) + list(np.array(permutations[cl])[(i+1)*14:])
                test_idxs[cl] = list(np.array(permutations[cl])[i*14: (i+1)*14])
        ...
        df_storage_train[lbl][i+1] = p_df['BinSim'].iloc[train_idxs]
        lbl_storage_train[lbl][i+1] = list(np.array(labels)[train_idxs])

        df_storage_test[lbl][i+1] = p_df['BinSim'].iloc[test_idxs]
        lbl_storage_test[lbl][i+1] = list(np.array(labels)[test_idxs])

        real_samples[lbl][i+1] = df_storage_test[lbl][i+1].copy()

# Linear interpolation of data
data, lbls = laf.artificial_dataset_generator(df_storage_train['FTMP'][1], labels=lbl_storage_train['FTMP'][1],
...
hist = np.histogram(real_samples['FTMP'][1].values.flatten(), bins=100)
...
def training_montage(train_data_o, train_lbls, test_data, test_lbls,
    ...
    for step in range(n_steps):
        ...
        if (step + 1) % update2 == 0:
            ...
            den, cov = gem.evaluation_coverage_density(test_data, saved_predictions[-1], k= k_cov_den, metric='jaccard')
...
for crit_n_str in test_nodes:
    ...
    generator = generator_model(data.shape[1], data.shape[1], gen_n, 3)
    critic = critic_model(data.shape[1], crit_n, 3)
    ...
    generator.save_weights('gan_models/MP_gen_opt_gen_'+ gen_nodes_str + 'crit_' + crit_n_str)
    #critic.save_weights('gan_models/MP_crit_example')
    ...
    with open('gan_models/MP_results_opt_gen_'+ gen_nodes_str + 'crit_' + crit_n_str + '.pickle', 'wb') as handle:
