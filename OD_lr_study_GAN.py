...
print(f"\nINFO: Job with array task id {idx} is using {lr_str} nodes in generator.")
...
for i in range(2):
    ...
    df_N = transf.normalize_sum(df_storage_train[i+1])
    df_G = transf.glog(df_N, lamb=None)
    real_samples[i+1] = transf.pareto_scale(df_G)

    test_datamatrix_N = transf.normalize_sum(df_storage_test[i+1])
    test_datamatrix_G = transf.glog(test_datamatrix_N, lamb=None)
    df_storage_test[i+1] = (test_datamatrix_G - df_G.mean())/np.sqrt(df_G.std())
    #df_storage_test[i+1] = transf.pareto_scale(transf.glog(test_datamatrix_N, lamb=None))
...
data, lbls = laf.artificial_dataset_generator(df_storage_train[1], labels=lbl_storage_train[1],
                                            ...
                                            binary_rnd_state=None, rnd_state=514)
...
generator.save_weights('gan_models/ODint_gen_opt_lr_'+ lr_str)
...
with open('gan_models/ODint_results_opt_lr_' + lr_str + '.pickle', 'wb') as handle:
